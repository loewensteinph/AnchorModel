...
namespace Anchor.Model.Core.BusinessLogic
{
    public class Anchor
    {
        ...
        private string metadataColumnName => $"{GlobalMetadata.MetadataPrefix}_{Mnemonic}";
        private string dummyColumnName => $"{Mnemonic}_{GlobalMetadata.DummySuffix}";
        private string businessIdentityColumnName => $"{Descriptor}_{businessIdentity}";
        ...
        public string CreatePitFunctionStatement => GetCreatePitFunctionStatement();
...
GO");
    ...
    [{0}].{1} = (
        ...
        internal string GetCreatePitFunctionStatement()
        {
            if (Attribute.Count == 0)
                return string.Empty;
            var result = string.Empty;
...
SELECT"
                , Capsule, TableName);
            sb.AppendFormat(@"[{0}].{1},
                [{0}].{2},", Mnemonic, identityColumnName, metadataColumnName);
            foreach (var attribute in Attribute)
            {
                var last = attribute.Equals(Attribute.Last());
                sb.AppendFormat(@"[{0}].{1},"
