...
BEGIN
    RETURN (CASE WHEN EXISTS (SELECT @value
                              WHERE  @value = (SELECT   TOP 1 pre.AC_NAM_Actor_Name
                                               FROM     [dbo].[AC_NAM_Actor_Name] AS pre
                                               WHERE    pre.AC_NAM_AC_ID = @id
                                                        AND pre.AC_NAM_ChangedAt < @changed
                                               ORDER BY pre.AC_NAM_ChangedAt DESC))
                      OR EXISTS (SELECT @value
                                 WHERE  @value = (SELECT   TOP 1 fol.AC_NAM_Actor_Name
                                                  FROM     [dbo].[AC_NAM_Actor_Name] AS fol
