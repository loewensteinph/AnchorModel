...
BEGIN
    RETURN (CASE WHEN EXISTS (SELECT @value
                              WHERE  @value = (SELECT   TOP 1 pre.PE_REV_Performance_Revenue
                                               FROM     [dbo].[PE_REV_Performance_Revenue] AS pre
                                               WHERE    pre.PE_REV_PE_ID = @id
                                                        AND pre.PE_REV_ChangedAt < @changed
                                               ORDER BY pre.PE_REV_ChangedAt DESC))
                      OR EXISTS (SELECT @value
                                 WHERE  @value = (SELECT   TOP 1 fol.PE_REV_Performance_Revenue
                                                  FROM     [dbo].[PE_REV_Performance_Revenue] AS fol
