...
RETURN 
    ...
    FROM   [dbo].[AC_Actor] AS [AC]
           LEFT OUTER JOIN
           [dbo].[rAC_NAM_Actor_Name](@changingTimepoint) AS [NAM]
           ON [NAM].AC_NAM_AC_ID = [AC].AC_ID
              AND [NAM].AC_NAM_ChangedAt = (SELECT max(sub.AC_NAM_ChangedAt)
                                            FROM   [dbo].[rAC_NAM_Actor_Name](@changingTimepoint) AS sub
           ...
           LEFT OUTER JOIN
           [dbo].[rAC_PLV_Actor_ProfessionalLevel](@changingTimepoint) AS [PLV]
           ON [PLV].AC_PLV_AC_ID = [AC].AC_ID
              AND [PLV].AC_PLV_ChangedAt = (SELECT max(sub.AC_PLV_ChangedAt)
                                            FROM   [dbo].[rAC_PLV_Actor_ProfessionalLevel](@changingTimepoint) AS sub
